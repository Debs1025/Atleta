...
export async function searchAthletes(queryStr?: string, sportType?: string) {
  ...
  for (const userDoc of usersSnapshot.docs) {
    ...
    if (role.includes('athlete') || role.includes('player') || role === 'user' || !u.role) {
      ...
      const rawDocs = Array.isArray(p.eligibility_documents)
        ? p.eligibility_documents
        : Array.isArray(u.eligibility_documents)
        ? u.eligibility_documents
        : [];

      const eligObj = (u.eligibility_documents && typeof u.eligibility_documents === 'object' && !Array.isArray(u.eligibility_documents))
        ? u.eligibility_documents
        : (p.eligibility_documents && typeof p.eligibility_documents === 'object' && !Array.isArray(p.eligibility_documents))
        ? p.eligibility_documents
        : null;

      const isVerified = Boolean(
        rawDocs.length > 0 ||
        eligObj?.psa_verified === true ||
        p.is_eligibility_verified === true ||
        u.is_eligibility_verified === true
      );

      const missingDocs: string[] = [];
      if (!isVerified) {
        if (!eligObj?.psa_verified) missingDocs.push('PSA Birth Certificate');
        if (!eligObj?.proof_of_residency) missingDocs.push('Proof of Residency');
        if (!eligObj?.academic_check) missingDocs.push('Academic Clearance');
        if (missingDocs.length === 0) missingDocs.push('Eligibility & Verification Documents');
      }

      const athleteObj: any = {
        athlete_id: athleteId,
        user_id: uid,
        first_name: firstName || 'Athlete',
        last_name: lastName || '',
  ...
  for (const profileDoc of profilesSnapshot.docs) {
    ...
    if (!resultsMap.has(athleteId)) {
      ...
      const eligObj = (p.eligibility_documents && typeof p.eligibility_documents === 'object' && !Array.isArray(p.eligibility_documents))
        ? p.eligibility_documents
        : (u.eligibility_documents && typeof u.eligibility_documents === 'object' && !Array.isArray(u.eligibility_documents))
        ? u.eligibility_documents
        : null;
      ...
      const searchHaystack = `${fullName} ${athleteObj.position} ${athleteId} ${p.email || ''}`.toLowerCase();
      if (!queryLower || searchHaystack.includes(queryLower)) {
        resultsMap.set(athleteId, athleteObj);
      }
    }
