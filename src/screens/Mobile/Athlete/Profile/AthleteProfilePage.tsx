...
export function AthleteProfilePage({
  ...
  const [avatarUri, setAvatarUri] = useState<string | undefined>(profile.avatar_url || (profile as any).avatar_uri);
  ...
  React.useEffect(() => {
    ...
    setAvatarUri(profile.avatar_url || (profile as any).avatar_uri);
  ...
  const bmi = bmiVal > 0 && !isNaN(bmiVal) ? bmiVal.toFixed(1) : "0.0";

  const apeRatioVal = heightCm > 0 && wingspanCm > 0 ? wingspanCm / heightCm : 0;
  const apeIndex = apeRatioVal > 0 && !isNaN(apeRatioVal) ? apeRatioVal.toFixed(2) : "0.00";
  const apeDiff = heightCm > 0 && wingspanCm > 0 ? wingspanCm - heightCm : 0;
  ...
  const handleSaveNewDocument = () => {
    ...
    const isDuplicate = documents.some(
      (d) =>
        d.title.trim().toLowerCase() === newDocTitle.trim().toLowerCase() ||
        (newDocAsset && d.fileName && d.fileName.toLowerCase() === newDocAsset.name.toLowerCase())
    );

    if (isDuplicate) {
      setAddDocError("A document with this name or file has already been added.");
      return;
    }

    const isPdf = newDocAsset?.name?.toLowerCase().endsWith(".pdf");
  ...
  const handleSave = async () => {
    ...
    const parsedWingspan = Number(wingspanCm);

    const safeHeight = !isNaN(parsedHeight) && parsedHeight >= 0 ? parsedHeight : (profile.height_cm ?? 0);
              ...
              <View style={styles.calculatedItem}>
                ...
                <Text style={styles.calcValue}>
                  ...
                  <Text style={styles.calcUnit}>
                    ({heightCm > 0 && wingspanCm > 0 ? (apeDiff >= 0 ? `+${apeDiff}` : apeDiff) : 0} cm reach)
                  </Text>
                </Text>
              </View>
            </View>
