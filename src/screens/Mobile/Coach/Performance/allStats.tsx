...
export const AllStats: React.FC<AllStatsProps> = ({
  ...
  const handleSetWorkloadTarget = async () => {
    ...
    try {
      await requestAuthenticatedJson("/analytics/workload", "POST", {
        athlete_id: athlete.athlete_id,
        target_7day_effort_pts: newTargetPts,
        target_intensity: targetIntensity,
        notes: `7-Day target set by coach (${newTargetPts} Effort Pts, Intensity ${targetIntensity}/10)`,
      });
    } catch (err) {
      console.warn("Workload target backend sync:", err);
    }
