...
export function SignupScreen({ onGoLogin }: SignupScreenProps) {
  ...
  const openCalendar = () => {
    const rawBirthdate = athleteForm.getValues("birthdate") || "";
  ...
  const handleSelectDay = (dayNum: number) => {
    ...
    athleteForm.setValue("birthdate", formatted, { shouldDirty: true, shouldValidate: true });
  ...
  const handleGoogleSignUpPress = async () => {
    ...
    try {
      ...
      if (res.type === "success" && (res.idToken || res.accessToken)) {
        ...
        setSocialToken({ provider: "google", idToken: token });
        setFeedback({
          tone: "info",
          message: `Google account connected! Please fill in your profile details below to complete your ${role.toUpperCase()} account.`
        });
        setStep(3);
        setLoading(false);
        ...
        setFeedback({
          tone: "error",
          message: res.error || "Google sign-up failed."
        });
      } else {
        setLoading(false);
      }
  ...
  const handleFacebookSignUpPress = async () => {
    setLoading(true);
    setFeedback(null);
    ...
    try {
      ...
      if (res.type === "success" && (res.accessToken || res.idToken)) {
        const token = (res.accessToken || res.idToken)!;
        setSocialToken({ provider: "facebook", idToken: token });
        setFeedback({
          tone: "info",
          message: `Facebook account connected! Please fill in your profile details below to complete your ${role.toUpperCase()} account.`
        });
        setStep(3);
        setLoading(false);
      } else if (res.type === "error") {
        ...
        setFeedback({
          tone: "error",
          message: res.error || "Facebook sign-up failed."
        });
      } else {
        setLoading(false);
      }
      ...
      setFeedback({
        tone: "error",
        message: e?.message || "Facebook sign-up failed."
      });
    }
  };
  ...
  const uploadDocument = async () => {
    ...
    const nextDocument: StoredUpload = {
      name: asset.name ?? (isPdf ? "document.pdf" : "eligible-document.png"),
      uri: asset.uri,
  ...
  return (
    <View style={{ flex: 1 }}>
          ...
          {step === 1 ? (
            <View>
              ...
              <Button
                ...
                onPress={handleFacebookSignUpPress}
              />
            </View>
          ) : null}
              ...
              <Pressable onPress={openCalendar}>
                <View pointerEvents="none">
                  <FormField
                    ...
                    rightAccessory={<MinimalistCalendarIcon />}
                    error={athleteErrors.birthdate?.message}
                    editable={false}
                  />
                </View>
              </Pressable>
      ...
      <DatePickerModal
        ...
        birthdate={athleteForm.watch("birthdate") || ""}
