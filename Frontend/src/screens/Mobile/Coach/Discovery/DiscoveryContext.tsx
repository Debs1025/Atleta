        ...
        if (isMounted) {
          if (Array.isArray(athletesRes) && athletesRes.length > 0) {
            const mapped: AthleteDiscoveryItem[] = athletesRes.map((a: any) => ({
              ...
              position_tag: a.position || (a.sport_type === 'SWIMMING' ? 'SWIMMING' : 'PG'),
              sport_category: (a.sport_type || a.sport_category || 'BASKETBALL').toUpperCase() as SportCategoryFilter,
              biometrics: {
                height_ft: a.physical_attributes?.height_cm
                  ? `${Math.floor(a.physical_attributes.height_cm / 30.48)}'${Math.round((a.physical_attributes.height_cm % 30.48) / 2.54)}"`
                  : `6'0"`,
                weight_lbs: a.physical_attributes?.weight_kg
                  ? `${Math.round(a.physical_attributes.weight_kg * 2.20462)} lbs`
                  : `175 lbs`,
                wingspan_ft: a.physical_attributes?.wingspan_cm
                  ? `${Math.floor(a.physical_attributes.wingspan_cm / 30.48)}'${Math.round((a.physical_attributes.wingspan_cm % 30.48) / 2.54)}"`
                  : `6'2"`,
              },
              stats: a.stats || { ppg: 0, rpg: 0, ast: 0, fg_pct: 0 },
              calculated_per: a.calculated_per || 28.5,
              efficiency_pct: a.efficiency_pct || 80,
              contact_info: {
          ...
          if (Array.isArray(proposalsRes)) {
            const mappedProposals: ScoutingProposalItem[] = proposalsRes.map((p: any) => ({
              ...
              athlete_name: p.athlete_details?.first_name
                ...
                : 'Athlete',
              sport_category: p.athlete_details?.sport_type || 'Basketball',
